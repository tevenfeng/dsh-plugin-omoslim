window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function SubagentModelsButton({ session, sessionId, useSessions }) {
			...
			// The composer's per-session snapshot has `sessionId`/`blank` but no
			// `agentPreset` (and no `id`) — agentPreset lives on the sessions
			// LIST store, the same source the agent-preset UI reads. `sessionId`
			// comes from the standard kit (standard.sessionId), guaranteed for
			// strict-session slot entries.
			const sid = sessionId !== void 0 ? sessionId : (session && session.sessionId);
			const agentPreset = useSessions((s) => (sid === void 0 ? void 0 : s.byId[sid]?.agentPreset));
