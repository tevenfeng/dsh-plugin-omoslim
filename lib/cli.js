 ...
 *
 * Switching re-renders agent.cordis.yml in place; the next session created in
 * dsh picks up the new models (no process restart required).
 */
...
function printProfiles() {
  ...
  for (const id of ids) {
    ...
    for (const name of profiles) {
      ...
      const slots = models ? Object.keys(models).length : 0;
      console.log(`  ${name}${mark}${models === undefined ? '  (unparsable)' : `  (${slots} slots)`}`);
    }
  }
  console.log('');
...
function switchTo(name) {
  ...
  console.log('create a new session in dsh to pick up the new models (no restart needed)');
}
...
usage:
  omoslim list               list profiles (active marked with *)
  omoslim current            print the active profile name
